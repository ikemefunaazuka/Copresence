 ...
 */
export function handleCursor(ctx: ConnectionContext, msg: CursorMessage, deps: ControllerDeps): void {
...
export function handleScroll(ctx: ConnectionContext, msg: ScrollMessage, deps: ControllerDeps): void {
  const session = deps.registry.get(ctx.sid);
  if (!session) return;
  const { state: next } = applyEvent(session, msg);
  deps.registry.save(next);
