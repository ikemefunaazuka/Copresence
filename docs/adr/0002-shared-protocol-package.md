...
The cost is a small one: the client and the server must both take a real dependency on this package, and — as built in Phase 1 — anything that touches it (lint, typecheck, a fresh test run) must ensure the package is actually built first, since it is consumed through its compiled output like any other npm dependency, not through source directly. That ordering problem is exactly what Phase 1's `pre<script>` npm hooks exist to solve, and is worth naming here because it is a direct, mechanical consequence of this decision, not an unrelated build quirk.
