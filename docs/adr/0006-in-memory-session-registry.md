...
The audit log (`AuditLog`, Phase 2) is a different case from presence data — see MILESTONE §3 — and is not what this ADR is about: an audit record exists precisely so it outlives the session, which is a real argument for durability that presence state does not share. Phase 2's `AuditLog` is in-memory for now anyway, matching the same "no database needed yet" posture, with its own upgrade path (a JSONL-backed implementation behind the same shape) explicitly reserved for Phase 6 rather than built early against a need that phase hasn't arrived at yet.
